...
- [ ] 2. Write a program that declares an integer, a character, and a float, assigns them values, and prints the values and their sizes using `sizeof`.
...
- [ ] 16. Write a `Makefile` with rules to compile your multi-file project, including a `clean` rule to remove object files and the executable.
- [ ] 17. Write a function that attempts to modify a `const` variable and observe the compiler error.
